...
export async function drawModCard(url) {
...

    {
      ...
      if (fillRate !== '--') nativeStatsItems.push({ l: '填充率', v: fillRate });
      if (yIndex) nativeStatsItems.push({ l: '昨日指数', v: yIndex });
      ...
      const measureDescNodes = () => {
        ...
        for (const node of descNodes) {
          if (node.type === 't') {
            ...
            h += wrapText(dummy, node.val, 0, 0, innerW, lh, 5000, false) + (isHeader ? 12 : 10);
          } else if (node.type === 'li') {
            dummy.font = `500 15px "${font}"`;
            h += wrapText(dummy, node.val, 0, 0, innerW - 24, 25, 5000, false) + 10;
          } else if (node.type === 'tb') {
            h += (measureTableLayout(dummy, node, innerW, 22, `500 14px "${font}"`, `800 14px "${font}"`)?.totalH || 0) + 16;
          } else if (node.type === 'g') {
            for (const item of node.items || []) {
              if (item.error || !item.imgCache) h += 72;
              else {
                const captionH = item.caption ? wrapText(dummy, item.caption, 0, 0, innerW, 22, 4, false) : 0;
      ...
      const measureVersions = () => {
        ...
        versions.forEach(v => {
          ...
          h += Math.max(26, wrapText(dummy, v.v, 0, 0, innerW - labelW - 12, 22, 200, false)) + 8;
        });
        return h;
      };
      ...
      const titleH = wrapText(dummy, title, 0, 0, headerTextW, 36, 3, false);
      dummy.font = `500 15px "${font}"`;
      const subH = subTitle ? wrapText(dummy, subTitle, 0, 0, headerTextW, 22, 2, false) + 8 : 0;
      ...
      const canvas = createCanvas(nativeWidth, totalH);
      ...
      ctx.fillStyle = '#102f3d';
      ctx.font = `800 30px "${font}"`;
      ty = wrapText(ctx, title, tx, ty, headerTextW, 36, 3, true) + 4;
      if (subTitle) {
        ctx.fillStyle = '#607d8b';
        ctx.font = `500 15px "${font}"`;
        ty = wrapText(ctx, subTitle, tx, ty, headerTextW, 22, 2, true) + 8;
      }
      if (authors.length) {
        let ax = tx;
        for (const author of authors.slice(0, 3)) {
          ...
          ctx.font = `700 13px "${font}"`;
          let name = author.n;
          while (ctx.measureText(name).width > 120 && name.length > 2) name = `${name.slice(0, -2)}...`;
          ctx.fillText(name, ax + 30, ty + 5);
          ax += ctx.measureText(name).width + 46;
          if (ax > hx + innerW - 80) break;
        }
        ty += 34;
      }
      if (tags.length) {
      ...
      if (statsH) {
        ...
        nativeStatsItems.forEach((s, i) => {
          ...
          roundRect(ctx, sx, sy, itemW, 72, 10);
          ctx.fill();
      ...
      if (propsH) {
        ...
        props.forEach((p, i) => {
          ...
          let val = p.v;
          while (ctx.measureText(val).width > colW - labelW && val.length > 4) val = val.slice(0, -1);
      ...
      if (versionH) {
        ...
        versions.forEach(v => {
          ...
          vy = wrapText(ctx, v.v, margin + cardPad + labelW + 12, vy, innerW - labelW - 12, 22, 200, true) + 8;
        });
        if (links.length) {
          vy += 4;
      ...
      if (descH) {
        ...
        for (const node of descNodes) {
          if (node.type === 't') {
            ...
            dy = await drawTextWithTwemoji(ctx, node.val, margin + cardPad, dy, innerW, isHeader ? 30 : 25, 5000, true) + (isHeader ? 12 : 10);
          } else if (node.type === 'li') {
            ctx.fillStyle = '#2f9ab7';
            ...
            ctx.fillStyle = '#263238';
            ctx.font = `500 15px "${font}"`;
            dy = await drawTextWithTwemoji(ctx, node.val, margin + cardPad + 24, dy, innerW - 24, 25, 5000, true) + 10;
          } else if (node.type === 'tb') {
            const tableH = drawTable(ctx, node, margin + cardPad, dy, innerW, 22, `500 14px "${font}"`, `800 14px "${font}"`, {
              headerBg: '#e8f4f8',
              cellBg: '#ffffff',
              border: '#c7dbe3',
              text: '#263238',
            ...
            for (const item of node.items || []) {
              if (item.error || !item.imgCache) {
                ctx.fillStyle = '#f3f8fb';
                roundRect(ctx, margin + cardPad, dy, innerW, 56, 8);
                ctx.fill();
                ctx.fillStyle = '#78909c';
                ctx.font = `13px "${font}"`;
              ...
              ctx.save();
              roundRect(ctx, ix, dy, item.dw, item.dh, 8);
              ctx.clip();
              ...
              if (item.caption) {
                ctx.fillStyle = '#607d8b';
                ctx.font = `13px "${font}"`;
                dy = wrapText(ctx, item.caption, margin + cardPad, dy, innerW, 22, 4, true) + 10;
              } else {
                dy += 10;
              }
            }
            ...
            if (node.imgCache && !node.imgFailed) {
              ...
              ctx.save();
              roundRect(ctx, ix, dy, node.dw, node.dh, 8);
              ctx.clip();
      ...
      ctx.font = `12px "${font}"`;
      ctx.textAlign = 'center';
      ctx.fillText('mcmod.cn | Generated by Koishi | Plugin By Mai-xiyu', nativeWidth / 2, totalH - 24);
